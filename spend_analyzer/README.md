...
| `ledger.py` | Load the optional external budget ledger CSV (Budget tab's alternative source). |
| `views/` | `drilldown.py`, `budget.py`, `merchants.py`, `cashflow.py`, `qc.py`. |
...
### Budget tab: optional external categorization

By default the **Budget** tab measures actuals with this app's built-in `tier1`
taxonomy. If your established budget uses a *different* category set, point the tab at
an externally pre-categorized **ledger** CSV whose `Category` column already matches
your `budget.yaml` goal keys — add a `ledger.csv` to the personal `budget.yaml`:

```yaml
# <data root>/spend_analyzer/config/budget.yaml
ledger:
  csv: spend_analyzer/data/budget_ledger.csv   # relative paths resolve from the data root
```

Its mere presence is the switch (remove the block to return to the `tier1` logic). The
ledger is regenerated by the `../finance_pipeline` orchestrator's optional convert step
(see its README) from the same categorized store — so the Budget tab and any
sheet/export built from that ledger agree. The ledger carries only date, person
(`Source`), category, and amount, so **only the date-range and person filters apply** to
this view (account type / channel / necessity don't). A configured-but-missing CSV
warns and falls back to the built-in categorization rather than erroring.
