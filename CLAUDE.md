...
| `finance_pipeline/` | `./venv` | `./run.py [--no-drive --llm --llm-defer --no-convert --no-ui --push-data]`; `tools/migrate_multiuser.py` | `./venv/bin/python -m pytest` |
...
- **Optional external converter → Budget tab**: when configured (see Data root),
  the pipeline's step 2b runs the converter's `refresh.py --no-fetch --all
  --no-upload` to write `budget_ledger.csv`; the analyzer's Budget tab reads it
  iff `ledger.csv` is set in the personal `budget.yaml` (else built-in tier1). Two
  separate switches (stdlib pipeline pointer vs analyzer YAML) by design — keep
  them pointing at the same CSV. `--no-fetch` avoids run.py↔refresh.py recursion;
  `--no-upload` keeps it off the network; the step is non-fatal (derives a view).
