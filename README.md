...
> Every figure below is **synthetic demo data** — invented people, banks, and merchants
> generated by
> [`spend_analyzer/tools/gen_demo_data.py`](spend_analyzer/tools/gen_demo_data.py). No real
> financial data appears anywhere in this repo. To reproduce them locally:
> `python -m tools.gen_demo_data --out /tmp/demo` then run the UI with
> `SPEND_VISUALIZER_DATA=/tmp/demo`.

**Overview** — faceted category table, headline KPIs, and a necessity → category sunburst:

![Spending overview](docs/img/overview.png)

**Budget vs. actual** — monthly goals against trailing averages, with a per-category trend:

![Budget vs actual](docs/img/budget.png)

**Merchants & recurring** — top merchants, per-merchant history, and transaction-level detail:

![Merchants and recurring](docs/img/merchants.png)
