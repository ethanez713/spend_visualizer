...
@pytest.fixture
def world(tmp_path):
    """A pre-migration tree: repo .secrets + an external data root, fake values only."""
    tsec = tmp_path / "repo" / "transactions" / ".secrets"
    tdata = tmp_path / "data" / "transactions" / "data"
    cdata = tmp_path / "data" / "plaid_category_transformer" / "data"
...
def _run(world, *extra):
    return main(["--root", str(world / "repo"), "--data-root",
                 str(world / "data"), "--owner", "Alice", *extra])


def _read_all(world):
    tokens = json.loads(
        (world / "repo" / "transactions" / ".secrets" / "tokens.json").read_text())
    with lzma.open(world / "data" / "transactions" / "data" / "transactions_raw.jsonl.xz",
                   "rt", encoding="utf-8") as f:
        raw = [json.loads(l) for l in f if l.strip()]
    durable = [json.loads(l) for l in
               (world / "data" / "transactions" / "data" / "transactions.jsonl")
               .read_text().splitlines() if l.strip()]
    categorized = [json.loads(l) for l in
                   (world / "data" / "plaid_category_transformer" / "data" /
...
def given_secret_files_when_migrated_then_perms_preserved(world):
    _run(world, "--yes")
    for rel in (("repo", "transactions", ".secrets", "tokens.json"),
                ("data", "transactions", "data", "transactions_raw.jsonl.xz"),
                ("data", "transactions", "data", "transactions.jsonl")):
...
def given_record_owned_by_someone_else_when_migrated_then_aborts_untouched(world):
    durable = world / "data" / "transactions" / "data" / "transactions.jsonl"
...
def given_token_owned_by_someone_else_when_migrated_then_aborts(world):
    tokens = world / "repo" / "transactions" / ".secrets" / "tokens.json"
...
def given_missing_files_when_migrated_then_skipped_not_crashed(world, capsys):
    ...
    tokens, raw, durable, _ = (json.loads(
        (world / "repo" / "transactions" / ".secrets" / "tokens.json").read_text()), None, None, None)
...
def given_bad_owner_name_when_migrated_then_rejected(world):
    with pytest.raises(SystemExit):
        main(["--root", str(world / "repo"), "--data-root", str(world / "data"),
              "--owner", "../evil", "--yes"])
